...
class UnderwaterImageProcessor:
    ...
    def __init__(self, contrast_limit=2.5, red_boost=1.3, blue_boost=1.2, 
                 ...
                 color_balance_intensity=0.5, white_balance_strength=0.4,
                 detail_enhancement=1.2):
        """
        Initialize the underwater image processor with parameters to control color restoration
        ...
        Args:
            ...
            color_balance_intensity: Strength of color balancing effect
            white_balance_strength: Strength of white balancing
            detail_enhancement: Strength of detail enhancement
        """
        ...
        self.color_balance_intensity = color_balance_intensity
        ...
        self.clahe = cv2.createCLAHE(clipLimit=contrast_limit, tileGridSize=(8, 8))
    ...
    def _create_ffmpeg_extraction_command(self, input_path, frames_path, num_workers):
        ...
        return [
            'ffmpeg', '-i', input_path, '-qscale:v', '1', 
            '-threads', str(num_workers),
            '-qmin', '1', '-qmax', '1',  # Force maximum quality for extracted frames
            frames_path
        ]

    def _create_ffmpeg_encoding_command(self, frames_path, output_path, fps, width, height, num_workers, crf=23):
        """Create FFmpeg command for encoding frames into video"""
        return [
            'ffmpeg', '-r', str(fps),
            '-i', frames_path,
            '-c:v', 'libx265',  # Use HEVC codec instead of H.264
            '-preset', 'slow',  # Use slow preset for better quality (vs medium)
            '-crf', str(crf),  # Lower CRF value for higher quality (23 is visually lossless)
            '-pix_fmt', 'yuv420p',
            '-threads', str(num_workers),
            '-x265-params', 'bframes=8:psy-rd=1:aq-mode=3:aq-strength=0.8:deblock=1,1',  # Advanced encoding parameters for better detail preservation
    ...
    def restore_color(self, image):
        ...
        # Boost brightness - more conservative to match target
        ...
        # Step 6: Apply detail enhancement for sharper image
        if self.detail_enhancement > 1.0:
            adjusted_bgr = self._enhance_details(adjusted_bgr)
        
        # Step 7: Apply final tone mapping to better match target video
    ...
    def _enhance_details(self, image):
        ...
        detail_layer = cv2.addWeighted(image, 1.5, blurred, -0.5, 0)
        
        # Combine original and enhanced details
        enhanced = cv2.addWeighted(
            image, 1.0, 
            detail_layer - image, self.detail_enhancement - 1.0, 
            0
        )
    ...
    def _process_frame(self, frame_path, output_frame_path):
        ...
        # Save with higher quality parameters to prevent compression artifacts
        cv2.imwrite(output_frame_path, corrected, [cv2.IMWRITE_JPEG_QUALITY, 95, cv2.IMWRITE_JPEG_PROGRESSIVE, 1, cv2.IMWRITE_JPEG_OPTIMIZE, 1])
        return True
    ...
    def process_video(self, input_path, output_path, fps=None, num_workers=None, crf=23):
        """
    ...
    def process_batch(self, input_folder, output_folder, fps=None, num_workers=None, crf=23):
        """
...
def main():
    parser = argparse.ArgumentParser(description="Enhance underwater images and videos by restoring natural colors (v3).")
    ...
    parser.add_argument("--contrast", "-c", type=float, default=2.5, help="CLAHE clip limit for contrast enhancement")
    parser.add_argument("--red", "-r", type=float, default=1.3, help="Red channel boost factor")
    parser.add_argument("--blue", "-b", type=float, default=1.2, help="Blue channel boost factor")
    parser.add_argument("--green", "-g", type=float, default=0.95, help="Green channel multiplier")
    parser.add_argument("--saturation", "-s", type=float, default=0.55, help="Saturation factor (1.0 = no change)")
    parser.add_argument("--brightness", "-B", type=float, default=1.05, help="Brightness boost factor")
    parser.add_argument("--color-balance", "-cb", type=float, default=0.5, help="Color balance intensity")
    parser.add_argument("--white-balance", "-wb", type=float, default=0.4, help="White balance strength")
    parser.add_argument("--detail", "-d", type=float, default=1.2, help="Detail enhancement factor")
    ...
    parser.add_argument("--quality", "-q", type=int, default=23, help="Video quality (CRF value: 18-28 range, lower = higher quality and larger size)")
    ...
    logger.info("Underwater Footage Converter v3.0")
    ...
    processor = UnderwaterImageProcessor(
        ...
        color_balance_intensity=args.color_balance,
    ...
    logger.info(f"Processing parameters: contrast={args.contrast}, red={args.red}, blue={args.blue}, " +
                ...
                f"color_balance={args.color_balance}, white_balance={args.white_balance}, " +
                f"detail={args.detail}, video quality={args.quality}")
