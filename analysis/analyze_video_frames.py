...
# Set the path to the extracted frames directory
frames_dir = 'comparison_videos/frames'

# Define frame indices to analyze
frames_to_analyze = [1, 2, 3, 4, 5, 6, 7, 8]
results = []

with PdfPages('video_analysis_result.pdf') as pdf:
    for frame_idx in frames_to_analyze:
        original_path = os.path.join(frames_dir, f'original_{frame_idx:04d}.jpg')
        script_path = os.path.join(frames_dir, f'img1_{frame_idx:04d}.jpg')
        reference_path = os.path.join(frames_dir, f'img2_{frame_idx:04d}.jpg')
        ...
        if os.path.exists(original_path) and os.path.exists(script_path) and os.path.exists(reference_path):
            ...
            fig, metrics = analyze_frame_triplet(original_path, script_path, reference_path, frame_idx)
...
if results:
    ...
    1. Current red_boost parameter is too high
    2. Set blue_boost parameter to: {ideal_blue_boost:.2f}
    3. Set green_boost parameter to: {ideal_green_boost:.2f}
    4. Set red_boost parameter to: {ideal_red_boost:.2f}
    
    Notes: 
    - The current script output appears too red compared to the reference.
    - The reference has a more balanced color profile.
    """
    
    print(summary)
    
    # Save summary to file
    with open('video_analysis_summary.txt', 'w') as f:
        f.write(summary)
else:
